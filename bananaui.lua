...
if isfile and readfile then
    if isfile(CONFIG_FILE) then
        ...
        if ok and raw and raw ~= "" then
            local fn, err = loadstring(raw, "@" .. CONFIG_FILE)
            if fn then
                local ok2, result = pcall(fn)
                if ok2 and type(result) == "table" then
                    -- Deep merge file ngoài lên DEFAULT_CONFIG
        ...
        if writefile then
            ...
            table.insert(lines, "        HopWhenIdle = true,")
            table.insert(lines, "        AutoHop = true,")
            table.insert(lines, "        AutoHopDelay = 60 * 60,")
            table.insert(lines, "        FpsBoost = false,")
            table.insert(lines, "        blackscreen = false,")
            table.insert(lines, "        LowGraphics = true,")
            table.insert(lines, "    },")
            table.insert(lines, "    Items = {")
            table.insert(lines, "        AutoFullyMelees = true,")
            table.insert(lines, "        Saber = true,")
            table.insert(lines, "        CursedDualKatana = true,")
            table.insert(lines, "        SoulGuitar = true,")
            table.insert(lines, "        RaceV2 = true,")
            table.insert(lines, "        AutoRaceV3 = true,")
            table.insert(lines, "        AutoRandomFruit = false,")
            table.insert(lines, "    },")
            table.insert(lines, "    Sword = {")
            table.insert(lines, '        ["Shark Saw"]        = true,')
            table.insert(lines, '        ["Wardens Sword"]    = true,')
            table.insert(lines, '        ["Pole (1st Form)"]  = true,')
            table.insert(lines, '        ["Gravity Blade"]    = true,')
            table.insert(lines, '        ["Longsword"]        = true,')
            table.insert(lines, '        ["Rengoku"]          = true,')
            table.insert(lines, '        ["Flail"]            = true,')
            table.insert(lines, '        ["Twin Hooks"]       = true,')
            table.insert(lines, "    },")
            table.insert(lines, "    BossWeapons = {")
            table.insert(lines, '        ["Awakened Ice Admiral"] = true,')
            table.insert(lines, '        ["Tide Keeper"]          = true,')
            table.insert(lines, '        ["Deandre"]              = true,')
            table.insert(lines, '        ["Urban"]                = true,')
            table.insert(lines, '        ["Diablo"]               = true,')
            table.insert(lines, '        ["Soul Reaper"]          = true,')
            table.insert(lines, '        ["Cake Prince"]          = true,')
            table.insert(lines, '        ["Core"]                 = true,')
            table.insert(lines, '        ["Darkbeard"]            = true,')
            table.insert(lines, '        ["Katakuri"]             = true,')
            table.insert(lines, '        ["Beautiful Pirates"]    = true,')
            table.insert(lines, "    },")
            table.insert(lines, "    Melee = {")
            table.insert(lines, "        AutoBuy              = true,")
            table.insert(lines, "        CheckMasteryAfterBuy = true,")
            table.insert(lines, "        RaidAtV1Mastery      = 500,")
            table.insert(lines, "        GodhumanAtV2Mastery  = 400,")
            table.insert(lines, "    },")
            table.insert(lines, "    AutoKen = true,")
            table.insert(lines, "    BringMobs = true,")
            table.insert(lines, "    PanicMode = {")
            table.insert(lines, "        Enabled          = true,")
            table.insert(lines, "        LowHealthPercent = 20,")
            table.insert(lines, "        SafeHealthPercent = 75,")
            table.insert(lines, "        EscapeHeight     = 2000,")
            table.insert(lines, "        CheckInterval    = 1,")
            table.insert(lines, "    },")
            table.insert(lines, "    Settings = { StayInSea2UntilHaveDarkFragments = true },")
            table.insert(lines, "    AutoSea2 = true,")
            table.insert(lines, "    AutoSea3 = true,")
...
function hoangtuveu()
    ...
    scrollLayout.Parent = TypeAccountScroll

    local function mkLabel(name, pos, text)
    ...
    BeliLabel = mkLabel("BeliLabel", UDim2.new(0.07, 0, 0.55, 0), "Beli: N/A")
    LevelLabel = mkLabel("LevelLabel", UDim2.new(0.07, 0, 0.35, 0), "Level: N/A    Third Sea : " .. utf8.char(0x274C))
    RaceLabel = mkLabel("RaceLabel", UDim2.new(0.07, 0, 0.45, 0), "Race: N/A")
    GodHumanLabel = mkLabel("GodHumanLabel", UDim2.new(0.07, 0, 0.80, 0), utf8.char(0x1F534) .. " GodHuman")
    PullLeverLabel = mkLabel("PullLeverLabel", UDim2.new(0.75, 0, 0.90, 0), utf8.char(0x1F534) .. " Pull Lever")
    ValkyrieHelmLabel = mkLabel("ValkyrieHelmLabel", UDim2.new(0.75, 0, 0.80, 0), utf8.char(0x1F534) .. " Valkyrie Helm")
    MirrorFractalLabel = mkLabel("MirrorFractalLabel", UDim2.new(0.40, 0, 0.90, 0), utf8.char(0x1F534) .. " Mirror Fractal")
    SkullGuitarLabel = mkLabel("SkullGuitarLabel", UDim2.new(0.07, 0, 0.90, 0), utf8.char(0x1F534) .. " Skull Guitar")
    ...
    DropShadow.ZIndex = 0
    DropShadow.Image = "rbxassetid://110438945732939"
    ...
    DropShadow2Holder.Position = UDim2.new(0.5, 0, 0.05, 0)
    ...
    DropShadow2.Image = "rbxassetid://110438945732939"
    ...
    UnderStatus.BackgroundTransparency = 0.999
    ...
    ImageLabel.Image = "rbxassetid://110438945732939"
    ...
    TextButton.MouseButton1Down:Connect(function()
        ...
        blur.Size = (blur.Size == 24) and 0 or 24
    end)

    -- ─── W + SetText + SetTask (tương thích logic cũ) ───────────
    W.Instances['Task1'] = Top2
    W.Instances['Task2'] = UnderStatus
    W.Instances['MainTextLabel'] = Top2
    ...
    SetText = getgenv().SetText

    -- SetTask alias cho logic mới (MainTask/SubTask)
    getgenv().SetTask = function(key, text)
        if key == "MainTask" then
            Top2.Text = "Status: " .. tostring(text)
        elseif key == "SubTask" then
            UnderStatus.Text = "Status Farm: " .. tostring(text)
        elseif getgenv()._origSetTask_ref then
            getgenv()._origSetTask_ref(key, text)
        end
    end
    SetTask = getgenv().SetTask

    getgenv().alert = function() end
    W.SetText = getgenv().SetText
    W.ToggleUI = function() end
    W.ToggleInterface = function() end
    W.RegisterForBlur = function() end
    getgenv().LonelyW = W
    if ScriptStorage then ScriptStorage.Interface = W end
    ...
    ScriptStorage = {IsInitalized = false, PlayerData = {}, Melees = {}, CurrentMeleeData = {}, Enemies = {}, Tools = {}, Backpack = {}, IgnoreStoreFruits = {}, Connections = {LocalPlayer = {}}, Task = {}, Tracebacks = {}, TaskController = {}, TracebackUpdater = {}, Interface = getgenv().LonelyW or W, NPCs = {}, Map = {}}
    ...
    getgenv()._origSetTask_ref = function(J, W)
